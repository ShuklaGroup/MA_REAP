...
def clustering_MA(trajectories, trajectories_log, n_agents, n_select, n_clusters=None, max_frames=1e5, b=1e-4,
                  gamma=0.7, d=2):
    '''
    ...
    agent_index = []  # Array mapping a frame index in trajectory to its corresponding agent --> Should all be zeros --> Sanity check
...
def select_least_counts_MA(kmeans, X, agent_idx, n_agents, stakes_method, stakes_k=None, n_select=50):
    ...
    agent_stakes (np.ndarray): array of shape (n_agents, n_select). Entry agent_stakes[i, j] indicates the fraction of frames from candidate j that were generated by agent i.
    '''
...
def collect_initial_data(num_trajectories, traj_len, initial_positions, output_dir, output_prefix, top_file):
    ...
    basename = os.path.join(output_dir, output_prefix + 'agent_0_init_traj_{}.dcd')
    n_start = len(initial_positions)
    output_files = [basename.format(t) for t in range(num_trajectories * n_start)]
    print(output_files)

    # Create array of initial positions for spawn trajectories
    init_pos = [initial_positions[i] for i in range(n_start) for _ in range(num_trajectories)]

    spawn_trajectories(init_pos, top_file, n_steps=traj_len, output_files=output_files)

    # Create initial trajectory log
    trajectories_log = [[basename.format(t) for t in range(num_trajectories * n_start)]]
    print(trajectories_log)

    trajectories = [[]]
    for t in range(num_trajectories * n_start):
        fname = basename.format(t)
        data = project_dihedral_alanine_dipeptide(fname, top_file)
        trajectories[0].append(data)  # Forced into first agent for single agent version
...
def run_trial(initial_structures, epochs, output_dir='', output_prefix='', **kwargs):
    ...
    Args
    -------------
    initial_structures (list[str]): path to starting structures for simulations. Length of list must match number of agents.
    ...
    **kwargs: used to specify model hyperparameters. Must include following keys:
    ...
    n_agents = 1  # Forced in single agent run
    ...
    # assert(len(initial_structures) == n_agents) --> Not valid for single agent run
    initial_positions = []
    for fname in initial_structures:
        xyz = md.load(fname).xyz[0]
        initial_positions.append(frame_to_openmm(xyz))
    trajectories, trajectories_log = collect_initial_data(num_spawn, traj_len, initial_positions, output_dir,
                                                          output_prefix,
                                                          top_file)  # Function was adapted for single agent
    ...
    for e in range(epochs):
        ...
        # Clustering
        kmeans, X, agent_idx, file_names, frame_indices = clustering_MA(trajectories, trajectories_log, n_agents,
                                                                        n_select, max_frames=max_frames, b=b,
                                                                        gamma=gamma, d=d)

        # Select candidates
        central_frames, central_frames_indices, agent_stakes = select_least_counts_MA(kmeans, X, agent_idx, n_agents,
                                                                                      stakes_method, stakes_k=stakes_k,
                                                                                      n_select=n_select)
        ...
        for a in range(n_agents):
            ...
            indices = np.where(agent_idx == a)
            X_a = X[indices]
            individual_rewards = compute_structure_reward_MA_standard_Euclidean(X_a, central_frames, agent_stakes[a],
                                                                                weights[a],
        ...
        chosen_frames_indices, chosen_frames, executors = select_starting_points_MA_standard_Euclidean(X,
                                                                                                       central_frames,
                                                                                                       agent_idx,
                                                                                                       agent_stakes,
                                                                                                       weights,
        ...
        for ex in executors:
            fname = os.path.join(output_dir, output_prefix,
                                 'agent_{}_round_{}_traj_{}.dcd'.format(str(ex), round_num, str(count_trajs[ex])))
            count_trajs[ex] += 1
