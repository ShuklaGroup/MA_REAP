...
from utils import save_pickle, load_pickle, msm_from_mtx, run_monte_carlo_traj, map_state_to_coordinates, num_clusters, spawn_trajectories_MA
...
def clustering_MA(state_sequences, n_agents, mapping, n_clusters=None, max_frames=1e5):
    ...
    mapping (callable): maps MSM microstate to CV-space.
...
def select_least_counts_MA(kmeans, X, agent_idx, n_agents, mapping, n_select=50, stakes_method='percentage', stakes_k=None):
    '''
...
def compute_cumulative_reward_MA_standard_Euclidean(X, agent_idx, agent_stakes, central_frames_indices, n_select, n_agents, weights, which_agent, mapping):
    '''
...
def tune_weights_MA_standard_Euclidean(rewards_function, weights_a, delta=0.02):
    ...
    constraints.append({
     ...
     # This constraint makes the weights always positive
    constraints.append({
        'type': 'ineq', 
...
def select_starting_points_MA_standard_Euclidean(X, central_frames, central_frames_indices, agent_idx, agent_stakes, weights, n_agents, mapping, n_chosen=10):
    '''
...
def run_trial(msm, initial_states, epochs, output_dir='./', output_prefix='', **kwargs):
    ...
    **kwargs: used to specify model hyperparameters. Must include following keys:
    ...
    # Step 3: collect some initial data --> Modified for single agent run
    n_clusters = 0
    while n_clusters < n_select: # Make sure to have enough states before running REAP
        for s in initial_states:
            state_seq = run_monte_carlo_traj(msm, init_state=s, n_steps=traj_len)
            state_sequences[0].append(state_seq) # Single agent run
        n_clusters = num_clusters(state_sequences[0])
    ...
    for e in range(epochs):
        ...
        # Select candidates
        central_frames, central_frames_indices, agent_stakes = select_least_counts_MA(kmeans, X, agent_idx, n_agents, mapping, n_select=n_select, stakes_method=stakes_method, stakes_k=stakes_k) 
        ...
        for a in range(n_agents):
            ...
            R, reward_fun = compute_cumulative_reward_MA_standard_Euclidean(X, agent_idx, agent_stakes, central_frames_indices, n_select, n_agents, weights, a, mapping)
        ...
        if ((e+1) % 10 == 0): # Plotting parameters apply for Src example
            print("Running epoch: {}/{}".format(e+1, epochs))
            for a in range(n_agents):
                print(np.round(weights[a][:2], 2))
                CVs = mapping(np.concatenate(state_sequences[a]))
                
                plt.scatter(CVs[:, 0], CVs[:, 1], s=10*(3-2*a))
            
            target = mapping(np.asarray([0]))
