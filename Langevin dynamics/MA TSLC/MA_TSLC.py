...
def principal_components(points, d=None):
    ...
    To be used on the points belonging to each of the clusters.
...
def compute_Vi_all_clusters(kmeans, X, d):
    '''
    Computes the matrices Vi. These matrices are of shape (D, d) and contain the principal component decomposition of each cluster. 
    This function differs from compute_Vi in that it uses all clusters instead of the least count clusters only.
