    ...
    **kwargs: used to specify model hyperparameters. Must include following keys:
    ...
    for e in range(epochs):
        ...
        for i in range(n_agents):
            ...
            trajectories[i] = spawn_trajectories(0, 0, trajectories[i], new_starting_points, seed=None,
                                                 traj_len=traj_len, potential=potential)
        ...
        if debug:  # Only works for cross potential
            if (e % 25 == 0) or (e + 1 == epochs):
                ...
                for a in range(n_agents):
                    ...
                    plt.scatter(least_counts_points_log[a][-1][:, 0], least_counts_points_log[a][-1][:, 1], s=20,
                                alpha=0.2, c='purple')
                    plt.scatter(selected_structures_log[a][-1][:, 0], selected_structures_log[a][-1][:, 1], s=1,
                                alpha=1, c='black')
                plt.xlim([0, 2])
                plt.ylim([0, 2])
