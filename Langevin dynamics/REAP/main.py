...
from potentials import two_wells, two_wells_complex, four_wells_symmetric, four_wells_asymmetric
from potentials import two_wells_func, two_wells_complex_func, four_wells_symmetric_func, four_wells_asymmetric_func
...
kwargs = {
    'num_spawn': 10, # Number of trajectories spawn per epoch --> Half as MA REAP because it is run twice (once per independent agent)
...
for i in range(trials_per_task):
    run_trial(
        ...
        initial_positions = [[0.8,1,0], [1.2,1,0]], # Change according to number of agents and their starting coordinates
        epochs = 2, # Change # of epochs as needed
        output_dir = 'REAP_SA_trial_four_wells_symmetric', # Change output directory as needed
