...
def compute_agent_stakes(agent_indices, n_agents, method='percentage', k=None):
    ...
    try:
        assert (np.abs(stakes.sum() - 1) < 1e-8)
    except AssertionError as err:
        print("Stakes:", stakes)
        print("Method:", method)
...
def run_trial(potential, initial_positions, epochs, output_dir='', output_prefix='', **kwargs):
    ...
    **kwargs: used to specify model hyperparameters. Must include following keys:
    ...
    n_features = kwargs[
        'n_features']  # Number of variables in OP space (in this case it's x, y, z where z is irrelevant to the potential)
    ...
    for e in range(epochs):
        ...
        # Clustering
        kmeans, X, agent_idx = clustering_MA(trajectories, n_agents, n_select, max_frames=max_frames, b=b, gamma=gamma,
                                             d=d)
